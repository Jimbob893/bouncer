"""A scripted agent tries six purchases against a sample policy.
...
#: The policy the demo runs against. Inlined rather than read from disk so
#: `bouncer demo` works from an installed wheel, where examples/ does not ship.
DEMO_POLICY = """# The policy used by examples/demo.py.
#
...
def run(home: Path) -> int:
    ...
    policy_file = home / "policy.yaml"
    policy_file.write_text(DEMO_POLICY, encoding="utf-8")
    source = LocalFileSource(policy_file)
    ...
    banner()
    print(f"  {DIM('policy')}      {policy_file.name}")
